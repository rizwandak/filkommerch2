...
import type { CreateSaleInput, ProductWithVariants, Category, StoreSettings } from "@backend/server-actions";
import { createSale, getProducts, getCategories, getStoreSettings, createOrderAndPayment } from "@backend/server-actions";
    ...
    try {
      ...
      const recordSaleInDatabase = async (actualPaymentMethod?: string) => {
        const saleInput: CreateSaleInput = {
          ...
          payment_method: actualPaymentMethod || (paymentMethod === "cash" ? "Tunai" : "Online Payment"),
          items: cart,
          subtotal,
          discount,
          tax: 0,
        ...
        const paymentMethodLabel = actualPaymentMethod || (paymentMethod === "cash" ? "Tunai" : "Online Payment");
      ...
      <div className="flex w-[30%] flex-col bg-card border-l border-border">
        ...
        <div className="shrink-0 space-y-3 border-t border-border p-4 bg-cream/50">
          ...
          <Button
            ...
            size="lg"
          >
            {isProcessing ? "Memproses..." : paymentMethod === "online" ? "BAYAR SEKARANG" : "BAYAR & CETAK STRUK"}
          </Button>
        </div>
      </div>
    </div>
