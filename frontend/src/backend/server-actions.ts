...
// Import CSV Server Action
export const importOrdersServerAction = createServerFn({ method: "POST" })
  .validator((data: { campaignId?: number; rows: any[]; overwrite?: boolean }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      ...
      return { success: false, error: e.message || "Failed to import CSV" };
    }
  });

export const clearImportedOrdersServerAction = createServerFn({ method: "POST" })
  .validator((data: { campaignId: number }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      const res = await serverFetch(`${baseUrl}/api/admin/import/orders/${data.campaignId}`, {
        method: "DELETE",
      });
      return await res.json();
    } catch (e: any) {
      console.warn("clearImportedOrdersServerAction error:", e);
      return { success: false, error: e.message || "Failed to clear imported orders" };
    }
  });

// Vendor CRUD Server Actions
export const getVendorsServerAction = createServerFn({ method: "POST" }).handler(async () => {
  try {
    const baseUrl = getApiUrl();
...
export const createVendorServerAction = createServerFn({ method: "POST" })
  .validator((data: { name: string; contact_person?: string; phone?: string; email?: string; notes?: string }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      const res = await serverFetch(`${baseUrl}/api/admin/vendors`, {
...
export const updateVendorServerAction = createServerFn({ method: "POST" })
  .validator((data: { id: number; name: string; contact_person?: string; phone?: string; email?: string; notes?: string; is_active?: boolean }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
...
// Production Summary Server Action
...
// Vendor Orders Server Actions
export const getVendorOrdersServerAction = createServerFn({ method: "POST" }).handler(async () => {
  try {
    const baseUrl = getApiUrl();
...
export const createVendorOrderServerAction = createServerFn({ method: "POST" })
  .validator((data: { vendor_id: number; po_number?: string; deadline?: string; notes?: string; items: any[] }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      const res = await serverFetch(`${baseUrl}/api/admin/vendoring/orders`, {
...
// Financial Overview Server Action
...
// Batch Product Prices Server Actions
export const getBatchProductPricesServerAction = createServerFn({ method: "POST" })
  .validator((data: { campaignId: number }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      const res = await serverFetch(`${baseUrl}/api/admin/batch-prices/${data.campaignId}`);
      return await res.json();
    } catch (e: any) {
      console.warn("getBatchProductPricesServerAction error:", e);
      return { success: false, data: [] };
    }
  });

export const saveBatchProductPricesServerAction = createServerFn({ method: "POST" })
  .validator((data: { campaignId: number; prices: { product_id: number; selling_price: number; filkom_price?: number }[] }) => data)
  .handler(async ({ data }) => {
    try {
      const baseUrl = getApiUrl();
      const res = await serverFetch(`${baseUrl}/api/admin/batch-prices/${data.campaignId}`, {
      ...
      console.warn("saveBatchProductPricesServerAction error:", e);
      return { success: false, error: e.message || "Failed to save batch prices" };
    }
  });
