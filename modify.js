const fs = require('fs');
let content = fs.readFileSync('frontend/src/routes/product.$slug.tsx', 'utf-8');

// 1. State Replacement
content = content.replace(/  const reviewVariants = useMemo\(\(\) => \{[\s\S]*?\}, \[reviewsData.reviews, reviewFilterVariant\]\);/m, `  const [isReviewsOpen, setIsReviewsOpen] = useState(false);
  ...
  }, [reviewsData.reviews, reviewSort]);`);

// 2. Remove bottom Review Card
content = content.replace(/            \{\/\* REVIEW CARD SECTION \*\/\}[\s\S]*?            <\/div>\n\n          <\/div>\n\n          \{\/\* COLUMN 3/m, `          </div>\n\n          {/* COLUMN 3`);

// 3. Insert new accordion Review Card
const accordionCode = `
            {/* REVIEW CARD ACCORDION */}
            <div className="border-2 border-ink rounded-2xl bg-white overflow-hidden shadow-[4px_4px_0px_0px_rgba(27,27,27,1)] mt-6" id="ulasan-section">
              <button
                            ...
                            <div key={rev.id} className="py-4 space-y-2">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="flex text-amber-400">
                                  {[1, 2, 3, 4, 5].map((s) => (
                                    <Star key={s} className={\`w-3 h-3 \${s <= rev.rating ? "fill-amber-400 text-amber-500" : "text-gray-300 fill-gray-100"}\`} />
                                  ))}
                                </div>
...
content = content.replace(/            \{\/\* Product Variant Selectors \*\/\}/m, accordionCode);

fs.writeFileSync('frontend/src/routes/product.$slug.tsx', content);
